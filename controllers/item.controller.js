...
module.exports = {
  createItem: async (req, res) => {
    try {
      ...
      //cek jika image tidka diupload (req.file : mengambil input file)
      if (!req.file) {
        return res
          .status(400)
          .json(response(400, "Validasi Error", "Image not found"));
      }

      //proses menyimoan data melalui ORM sequelize
      const item = await Item.create({
        name: data.name, //ambi dari object data yang divalidasi sebelumnya
        stock: data.stock,
        image: req.file.filename, //ambil filename hasil dari middleware multer
      });
      return res.status(201).json(response(201, "created item", item));
    } catch (error) {
      //penanganan err kodingan di try
      //res : parameter func untuk memnerikan response(hasil)
      //response : method dari helpers formatter untuk format hasil outputnnya, output dalam bentuk json
  ...
  getItem: async (req, res) => {
    try {
      ...
      //sortBy : gurutin berdasarkan field apa
  ...
  updateItem: async (req, res) => {
    try {
      ...
      //bandingkan stok sebelumnya dan stok yang diinput
      if (!item) {
        return res
          .status(400)
